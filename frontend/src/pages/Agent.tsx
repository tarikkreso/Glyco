...
import { ArrowUp, ClipboardList, Database, ShieldCheck } from "lucide-react";
...
export function Agent() {
  ...
  const chat = useMutation({
    ...
    onSuccess: (response, message) => {
      setMessages((items) => [...items, { role: "user", content: message }, { role: "assistant", content: response.answer, response }]);
    },
  });
  const feedback = useMutation({
    ...
    onSuccess: () => {
      setMessages((items) => [...items, { role: "assistant", content: "Feedback saved. Glyco will use it to personalize the next answer." }]);
    },
  });
  ...
  return (
    <div className="page">
      <PageHeader title="Agent" subtitle="Ask natural-language questions while Glyco calls the risk model, trend model, logs, reports, and guidance tools." meta={`Mode: ${llmLabel}`} />
      <div className="agent-layout">
        <Card title="Glyco Agent" action={<Badge>{llmLabel}</Badge>}>
                ...
                {message.response && (
                  <div className="agent-evidence">
                    ...
                    <div className="tool-list">
                      {(message.response.tools_used?.length
                        ? message.response.tools_used.map((name) => `${name}: used`)
                        : message.response.tool_calls.map((tool) => `${tool.name}: ${tool.result_summary ?? tool.status}`))
          ...
          <form className="chat-compose" onSubmit={(event) => { event.preventDefault(); submit(); }}>
            <input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask Glyco about this week..." aria-label="Ask Glyco" />

...
Najvazniji backend tok pocinje u `app/api/routes.py`. Kada korisnik unese profil, backend racuna BMI, cuva profil i pokrece risk assessment. Kada korisnik unese novi health log, backend ga cuva, pokrece monitoring assessment i zatim proactive check koji moze kreirati alert. Agent chat endpoint prima pitanje korisnika i poziva `agent_service.py`, gdje se orkestriraju svi alati potrebni za odgovor.
...
Novi learning loop je implementiran preko tabele `agent_feedback`. Korisnik moze oznaciti da li je odgovor bio koristan, koji ton preferira i koju akciju potvrdjuje. Agent cita zadnjih 12 feedback zapisa i iz njih izracunava `learning_summary`: broj feedback signala, procenat korisnosti, preferirani ton i potvrdjene akcije. Taj summary se vraca frontend-u i koristi u narednom odgovoru. Time Glyco ima dokazivu online adaptaciju, a ne samo offline treniran model.

## 3. Masinsko ucenje i fallback

Risk model koristi BRFSS dataset i treniran je kao random forest classifier. Backend profil prevodi u isti feature oblik koji model ocekuje, zatim dobija vjerovatnocu i pretvara je u nivo rizika: low, medium ili high. Monitoring model koristi UCI time-series podatke. Logovi se agregiraju u dnevne karakteristike kao sto su prosjecna glukoza, standardna devijacija, minimum, maksimum, broj visokih ocitanja, broj niskih ocitanja, aktivnost i kratkorocni trend.
